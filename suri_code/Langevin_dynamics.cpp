...
void Langevin_dynamics::initialize(int N_max1, int N_max2, long int randomseed1, double S1, int inputsoft, double ksolute_solvent)
{
	...
	{
		vector< vector<double> > pos2; // A vector of vectors - 2d vector
		vector< vector<double> > fpos2;

		for (int loopj=0; loopj<Ntype[loopi]; loopj++)
		{
			vector<double> pos1; // initializing a vector of type double
			vector<double> fpos1;

			for(int loopk=0; loopk<2; loopk++)
			{
				pos1.push_back(gsl_rng_uniform(r)*Lx); // add value to end of vector pos1
				fpos1.push_back(0);
			}
			pos2.push_back(pos1); // add 1D vector pos1 to the end of vector pos2
			fpos2.push_back(fpos1);
		}
		pos.push_back(pos2); // Where is this declared?
...
double Langevin_dynamics::propagate_dynamics(double dt,double Pe, double t_c, double tau)
{
	double fd_term,noise_0,noise_1;
	double del1,del2,deltheta1;
	gamma_i=2;
