...
/**	@brief 	Extract and process data from output file previously created by Cytosim
	...
	@param	outputFileName			-	std::string
	...
	*/
void process_data_file( Simul & simul, std::string dataFileName, std::string outputFileName)
{
	...
	while (dataFile)
	{
		...
		while (std::getline(dataFile, line))
		{
			...
			else // If line DOES match endString
			{
				...
				outputFile << currentFrame.timeStamp ;
				outputFile << "\t" ;
				outputFile << currentFrame.work ;
				outputFile << "\n" ;
