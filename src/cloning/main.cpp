...
int main()
{
	...
	fs::path calculateExePath = rootPath / "calculate" ;
	std::string calculateExePathString = calculateExePath.string() ;

	printf("calculateExePathString: %s\n", calculateExePathString.c_str() ) ;
	...
	for (cloningParams.idxIter = 0 ; cloningParams.idxIter < cloningParams.numIters ; ++cloningParams.idxIter)
	{
		...
		/* Run simulations for each clone */
		for (int cloneIdx = 0 ; cloneIdx < cloningParams.numClones ; ++cloneIdx)
		{
			...
			get_dir_name(cloningParams, clone, cloneIdx) ;
			printf("clone.dirName: %s\n", clone.dirName.c_str()) ;

			clone.dirPath = rootPath / clone.dirName ;

			fs::current_path(clone.dirPath) ;

			std::cout << clone.dirPath << std::endl ;

			// change to the clone's directory
			// fs::current_path(clonePath) ;
			...
			calc_observable("calculate", calculateExePathString) ;
		...
		/* Calculate the sum of the s_a values for all clones */
